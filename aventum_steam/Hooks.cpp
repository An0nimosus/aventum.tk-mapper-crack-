	...
	LPCWSTR lpCurrentDir, LPSTARTUPINFOW pStartupInfo, LPPROCESS_INFORMATION pProcessInfo )
{
	...
	g_Utils.OpenBinary("C:\\csgo_7FA90000.bin", aBinary );

	void* pBase = VirtualAllocEx( hProcess, reinterpret_cast< void* >( 0x7FA90000 ), aBinary.size( ), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE );
	if ( !pBase )
	{
	...
	if (!VirtualAllocEx(hProcess, reinterpret_cast<void*>(0x35650000), 0x7C0000, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE))
	{
		printf("failed to alloc mem\n");
		return bResult;
	}
